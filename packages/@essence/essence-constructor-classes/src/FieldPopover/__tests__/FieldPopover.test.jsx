...
import FieldPopover, { FieldPopoverBase } from "../FieldPopover";
...
describe("FieldPopover", () => {
    ...
    it("render grid", () => {
        const wrapper = mountWithTheme(<FieldPopover bc={{ ...gridfieldJson, pagesize: 0 }} {...fieldConfig} />);

        expect(wrapper.find(FieldPopoverBase).exists()).toBeTruthy();
    ...
    it("render hidden", () => {
        const wrapper = mountWithTheme(<FieldPopover bc={gridfieldJson} {...fieldConfig} hidden />);

        expect(wrapper.find(SearchTextField).exists()).toBeFalsy();
    });

    it("render grid с установленным value = 4", async () => {
        const wrapper = mountWithTheme(<FieldPopover bc={gridfieldJson} {...fieldConfig} value="4" />);
        const store = wrapper.find(FieldPopoverBase).prop("store");
    ...
    it("Измнение значение через props", async () => {
        const wrapper = mountWithTheme(<FieldPopover bc={gridfieldJson} {...fieldConfig} />);
        const store = wrapper.find(FieldPopoverBase).prop("store");
    ...
    it.skip("Установка значение в first", async () => {
        const wrapper = mountWithTheme(<FieldPopover bc={{ ...gridfieldJson, autoload: "true" }} {...fieldConfig} />);
        const store = wrapper.find(FieldPopoverBase).prop("store");
